...
def monitor_memory(max_gb=2):
    """Check memory usage, exit if too high"""
    mem = psutil.Process().memory_info().rss / 1024**3
    if mem > max_gb:
        print(f"🚨 Memory limit exceeded: {mem:.1f}GB > {max_gb}GB")
        return False
    return True
...
def main():
    ...
    # Paths - On Windows
    source_dir = Path("./raw_data_wiki")
    xml_files = [f for f in source_dir.glob("plwiki-*.bz2") if "pages-articles" in f.name and "xml" in f.name]
...
💾 MEMORY USAGE:
Peak memory: {stats['memory_peak_gb']:.2f} GB
Available at end: {psutil.virtual_memory().available / 1024**3:.1f} GB
