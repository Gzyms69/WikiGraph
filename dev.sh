...
function start_backend() {
    ...
    if pgrep -f "uvicorn app.main:app" > /dev/null; then
        ...
        for i in {1..10}; do
            if curl -s "http://localhost:$BACKEND_PORT/api/health" | grep -q "status"; then
                echo -e " ${GREEN}OK${NC}"
                return
            fi
            echo -n "."
...
case "$CMD" in
    start)
        if [ -z "$ARG" ]; then
            echo "Usage: ./dev.sh start {pl|de|es|backend|all}"
            exit 1
        fi
    ...
    restart)
        if [ -z "$ARG" ]; then
             echo "Usage: ./dev.sh restart {pl|de|es|backend}"
             exit 1
        fi
    ...
    status)
        ...
        if pgrep -f "next dev" > /dev/null || pgrep -f "next start" > /dev/null; then
